...
export default function FgInventoryDetail({ inventory }) {
  ...
  useEffect(() => {
    if (!inventory?.productCode) return;

    const fetchDetail = async () => {
      try {
        const res = await InventoryAPI2.getFgInventoryDetail(
  ...
  const lotList = useMemo(() => {
    ...
    return detailData.histories.map((h, idx) => ({
      id: idx,
      lotNo: h.lot,
      qty: h.qty,
  ...
  //       type: "창고이동",
  //       qty: 0,
  //       location: "A-101 -> B-202",
  //       lotNo: "LOT-260125-11",
  //     },
  //     {
  //       id: 105,
  //       time: "2026-01-25 15:45",
  //       type: "반품입고",
  //       qty: 10,
  //       location: "R-001",
  //       lotNo: "LOT-260115-02",
  ...
  const historyColumns = [
    ...
    { key: "type", label: "구분", width: 100 },
    {
      ...
      width: 80,
      render: (val) => (
        <QtyText $isPositive={val >= 0}>{val > 0 ? `+${val}` : val}</QtyText>
      ),
    },
      ...
      <Content>
        <Section>
          ...
          <Grid>
            <FullItem>
              <label>제품코드</label>
              <Value>{detailData?.productCode}</Value>
            </FullItem>
            <FullItem>
              <label>제품명</label>
              <Value>{detailData?.productName}</Value>
            </FullItem>
            <FullItem>
              <label>총 재고</label>
              <Value>
                <strong>{detailData?.qty?.toLocaleString()}</strong>
                <Unit>{detailData?.unit}</Unit>
              </Value>
            </FullItem>
          </Grid>
        </Section>
